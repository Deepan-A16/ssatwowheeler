...
app.post('/api/parking/entry', async (req, res) => {
  try {
    ...
    const now = new Date();
    const dateStr = inDate || now.toLocaleDateString('en-GB');
    const timeStr = entryTime || now.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
...
app.post('/api/parking/checkout', async (req, res) => {
  try {
    ...
    if (entry) {
      const now = new Date();
      const exitDate = now.toLocaleDateString('en-GB');
...
// 9. Admin API: Get All PostgreSQL Tables & Column Metadata
...
// 10. Admin API: Get Rows from a Specific Table
app.get('/api/admin/tables/:tableName', async (req, res) => {
  try {
    ...
    const rowsRes = await db.query(`SELECT * FROM "${tableName}" ORDER BY 1 DESC LIMIT 200;`);
    const formattedRows = rowsRes.rows.map(formatEntryRow);
    res.json({ success: true, tableName, rows: formattedRows });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    ...
    console.log(`📊 Database Viewer available at http://localhost:${PORT}/db-viewer.html`);
  });
}
